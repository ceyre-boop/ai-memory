...
const MOCK_ENV = () => ({ ANTHROPIC_API_KEY: "test-key-123", AI_MEMORY_MODEL_URL: `http://127.0.0.1:${mock.port}/v1/messages` });
...
test("no args → usage; no key without --dry → clear error, no call", async () => {
  expect((await runAsk([])).code).not.toBe(0);
  const r = await runAsk(["sourdough"]);
...
test("default run expands with 3 variants, unions results, answers with cited sources", async () => {
  ...
  expect(r.out).toContain("— claude-opus-5 · ");
